...
extern void free();
void exit();
extern char FAR *strcat();
extern char FAR *strcpy();
extern char FAR *strncpy();
extern char FAR *strchr();
extern char FAR *strrchr();
extern int strlen();
extern int strcmp();
#ifdef VMS
#include <ssdef.h>
...
static void  crc_buf(char FAR *str, int len);
static void  strparity(uchar *p);
static FILE  FAR *fopen_msg(char *name, char *mode);
static int   fget_byte(FILE FAR *f);
static uint  fget_word(FILE FAR *f);
static ulong fget_longword(FILE FAR *f);
static void  fread_crc(uchar FAR *p, int n, FILE FAR *f);
...
static long  find_header(FILE FAR *fd);
static int   read_header(int first, FILE FAR *fd, char *name);
...
FILE FAR *f;
{
    n = file_read((char FAR *)p, 1, n, f);
    origsize += n;
    crc_buf((char FAR *)p, n);
}
...
int   n;
{
    ...
    {
        if (file_write((char FAR *)p, 1, n, outfile) != n)
            error(M_CANTWRIT, "");
    }
}
...
char *entry;
{
    ...
    {
    #ifdef Proolix
        if ((ptr = (char *)strrchr(pathname, *cptr)) != NULL &&
    #else
...
int size;
{
    ...
    if ((p = (char FAR *)xmalloc(size)) == NULL)
        error(M_NOMEMORY, "");
    return (voidp FAR *)p;
}
...
unstore()
{
    ...
    buffer = (char FAR *)malloc_msg(BUFFERSIZE);
    ...
    while (compsize > 0)
    {
        ...
        disp_clock();
        compsize -= n;
        fwrite_txt_crc((uchar FAR *)buffer, n);
