...
#if 0 // Á‡Ô‡Ò ÙÛÌÍˆËÈ, ÍÓÚÓ˚Â ÔÓÍ‡ ÌÂ ËÒÔÓÎ¸ÁÛ˛ÚÒˇ
...
/* œÓÔÛÒÍ ÒËÏ‚ÓÎÓ‚-Á‡ÔÓÎÌËÚÂÎÂÈ ‚ Ì‡˜‡ÎÂ ÒÚÓÍË */
while(*s1)
  {
  if (strchr(s2,*s1)==NULL)break;
  s1++;
...
while(i1<i2)
  {
  /* swap str[i1] str[i2] */
  /* —Ú‡Ì‰‡ÚÌ‡ˇ ÒıÂÏ‡ ÔÂÂÒ‡‰ÍË ÔË ÔÓÏÓ˘Ë ·ÓÎ‚‡Ì‡
  (C) ¿.ÃËÂ, "ƒÓÏ ÒÍËÚ‡Î¸ˆÂ‚" */
...
switch(c)
  {
  /*  (C)  —ÔËÒÓÍ ÒËÏ‚ÓÎÓ‚ ÔÛÌÍÚÛ‡ˆËË ‚ÁˇÚ ËÁ ÔÓ„‡ÏÏ˚ Connect ƒ.ŒÎÓ‚‡ Ë ‰.
...
/* »ÒÔÓÎ¸ÁÓ‚‡ÌËÂ ‚ Í‡˜ÂÒÚ‚Â ·ÛÙÂ‡ Ï‡ÒÒË‚‡ printf_buf, ‚˚‰ÂÎˇÂÏÓ„Ó ÔÓ
malloc (ÒÏ. ‚ kernel.c) ‚˚Á˚‚‡ÂÚ „Î˛ÍË (Á‡‚ËÒ‡ÌËˇ).
¿ ËÒÔÓÎ¸ÁÓ‚‡ÌËÂ ‚ Í‡˜ÂÒÚ‚Â ·ÛÙÂ‡
ÎÓÍ‡Î¸ÌÓ„Ó Ï‡ÒÒË‚‡ new_printf_buf „Î˛ÍÓ‚ ÌÂ ‚˚Á˚‚‡ÂÚ. ¬ÒÂ ‡·ÓÚ‡ÂÚ. */
...
char new_printf_buf[MAX_LEN_STR]; /* ÒÏ. ÔËÏÂ˜‡ÌËÂ ‚ Ì‡˜‡ÎÂ Ù-ˆËË printf */
  ...
  if (*format!='%')
    ...
    *str++=*format;
    ...
    while(strchr("-+ #0",c=*++format)!=NULL)
      switch (c)
      {
      /* ‰‡ÌÌ‡ˇ Ë‰ËÓÚÒÍ‡ˇ Á‡ÔËÒ¸ ‚Ë‰‡ "var=1; if (var)" ËÒÔÓÎ¸ÁÛÂÚÒˇ ‰Îˇ ÚÓ„Ó,
      ˜ÚÓ·˚ ÌÂ ·˚ÎÓ ÒÓÓ·˘ÂÌËˇ ÍÓÏÔËÎˇÚÓ‡ "variable not used". œÓÒÎÂ ÚÓ„Ó,
      Í‡Í Ó·‡·ÓÚÍ‡ ÙÎ‡„Ó‚ ·Û‰ÂÚ Â‡ÎËÁÓ‚‡Ì‡, Ë‰ËÓÚÒÍËÂ ÓÔÂ‡ÚÓ˚ ·Û‰ÛÚ,
      ÂÒÚÂÒÚ‚ÂÌÌÓ, Û‰‡ÎÂÌ˚ */
...
switch(c)
  ...
  case 0x8E: puts0("LVM    "); break; // –†–∞–∑–¥–µ–ª LVM
  #if 0
...
#if 1 // ƒÃ— œ‘Ã¡ƒÀ… œﬁ…›¡≈Õ N ¬ÃœÀœ◊ –œ”Ã≈ Àœ“Œ≈◊œ«œ À¡‘¡Ãœ«¡, ¡ ◊œœ¬›≈ ‹‘œ Œ≈ Œ’÷Œœ
...
if (FCB[0])
	{// »¡ŒƒÃ≈“ œ‘À“Ÿ‘
	// –“œ◊≈“—≈Õ, Œ¡ƒœ Ã… ƒœ¬¡◊Ã—‘ÿ ”Ã≈ƒ’¿›…  ¬ÃœÀ ◊ √≈–œﬁÀ’
	offset=(FCB[4]<<16)|FCB[3];
	//puts0("offset=");putdec(offset);puts0("\r\n");
	if (offset%499==0)
		{// Œ¡ƒœ ƒœ¬¡◊Ã—‘ÿ ¬ÃœÀ
		// …›≈Õ ”◊œ¬œƒŒŸ  ¬ÃœÀ Œ¡ ƒ…”À≈ (ƒœ –≈“◊œ  œ€…¬À…, Àœ‘œ“¡— Œ¡◊≈“Œœ≈ ÀœŒ≈√ ƒ…”À¡)
		// ≈›≈ Œ… œƒ…Œ ”◊œ¬œƒŒŸ  ¬ÃœÀ Œ≈ ◊Ÿƒ¡Œ?
			// ﬁ…‘¡≈Õ superblock
			i=secread(device, SUPERBLOCK, buffer512);
			if (i!=1) {puts0("Superblock read err\r\n"); return -1;}
			if ((buffer512[0]!=0xBE) || (buffer512[1])!=0xBE) // –“œ◊≈“—≈Õ magick
				{// ‹‘œ Œ≈ ”’–≈“¬ÃœÀ, ¡ »“≈Œÿ À¡À¡—-‘œ
			...
			if (end_formatted_bl==0)
				{// Œ≈ ¬ŸÃœ ∆œ“Õ¡‘…“œ◊¡Œ…—, ¬≈“≈Õ –≈“◊Ÿ  ¬ÃœÀ ⁄¡ Àœ“Œ≈◊ŸÕ À¡‘¡Ãœ«œÕ
				newbl=ROOT_DIR+1;
				...
				{// ¡ ‘≈–≈“ÿ …›≈Õ ”◊œ¬. ¬ÃœÀ ”Œ¡ﬁ¡Ã¡ –œ ∆œ“Õ¡‘…“œ◊¡ŒŒœ  œ¬Ã¡”‘… (–œ ƒ≈”À“…–‘œ“’)
				// …Ã… –œ‘œÕ ”“¡⁄’ ¬≈“≈Õ ”Ã≈ƒ. ¬ÃœÀ …⁄ Œ≈∆œ“Õ¡‘…“œ◊¡ŒŒœ  œ¬Ã¡”‘…
				newbl=0;
				...
				if (newbl==0)
					{// Œ≈ Œ¡€Ã… ”◊œ¬. ¬ÃœÀ ◊ œ‘∆œ“Õ¡‘…“œ◊¡ŒŒœ  œ¬Ã¡”‘…
					// ¬≈“≈Õ ”Ã≈ƒ ¬ÃœÀ –œ”Ã≈ œ‘∆œ“Õ¡‘…“œ◊¡ŒŒœ  œ¬Ã¡”‘…
		...
		// ÕŸ ƒœ¬¡◊Ã—≈Õ –≈“◊Ÿ  ¬ÃœÀ ∆¡ Ã¡?
		if (FCB[2]) // FCB[2] - ŒœÕ≈“ ‘≈À’›≈«œ ¬ÃœÀ¡
			{// ƒœ¬¡◊Ã—≈Õ Œ≈ –≈“◊Ÿ  ¬ÃœÀ
			// –…€≈Õ ””ŸÃÀ’ Œ¡ Œœ◊Ÿ  ¬ÃœÀ ◊ ‘≈À’›≈Õ ¬ÃœÀ≈
			...
			// –…€≈Õ Œ¡ ƒ…”À Œœ◊Ÿ  ¬ÃœÀ ” –≈“◊ŸÕ ⁄¡–…”¡ŒŒŸÕ ◊ Œ≈«œ ¬¡ ‘œÕ
			for (i=0;i<512;i++)buffer512[i]=0;
			buffer512[0]=1;
			buffer512[3]=c;
			offset++;
